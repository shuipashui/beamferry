...
| 项目 | 当前值 |
|---|---|
| Android 源码版本 | `0.8.127-quad-lane-rotation` |
| versionCode | `140` |
...
- `0.8.125` 移除设备不可用的 1080p 切换并固定 1920×1440。四码 50 与 60 都使用高帧率 AFL2 标记、固定四格缓存和同一诊断路径；高帧率格位锁定后不再被滚动快门残片拖动。单格加强阈值从 4 次连续失格放宽到 12 次，并移除屏幕二维码无意义的反色尝试。
- `0.8.126` 回滚 0.8.125 错误的理论中心刚性四格，恢复按实际命中框跟随；自动重定相阈值收紧至低于 0.20 QR/frame，避免中等相位重绑后恶化。一帧内多码改为单批次进入协议执行器，诊断增加协议批次。原生 C++ 单 JNI 四 ROI 作为下一阶段独立改动，不与本次回归修复叠加。
- `0.8.127` 四码整屏刷新时每屏轮转“序号到物理格位”的映射，避免一个长期盲格永久吞掉同一 modulo-4 systematic 序号通道并导致 LT 解块长停顿。APK 进度详情同时显示唯一包百分比和解块百分比，用于区分光学停顿与喷泉码级联停顿。
- `0.8.128` 修复高速四码“稳定缓存”仍被每帧稀疏命中覆盖的问题：四格建立后固定 ROI，短时全空帧也不再清除格框，避免滚动快门只照亮一侧时格框逐步漂移并造成数秒光学空窗。普通四码 30 FPS 仍使用原跟踪与失锁逻辑。
- `0.8.129` 针对实测末尾 38 帧、7.2 秒无 AFL2 的坏相位死等：早期低命中重定相仍限一次；传输中后期同时出现至少 24 个连续空帧且 1.2 秒无有效帧时允许受控重定相，总上限三次。协议解块进度保留，普通四码 30 FPS 不触发。
- `0.8.130` 修复高速四码只命中一个格位时就把四格缓存视为稳定的问题。四个物理格位分别记录真实命中校准状态；已校准格位冻结，未校准格位继续跟随补扫结果，诊断增加“格位校准 N/4”。普通四码 30 FPS 不进入该状态。
- `0.8.131` 在高速四码校准不足 4/4 时每 8 个分析帧对未校准格位执行一次定向宽 ROI 探测，即使其他格位仍持续命中也会补扫；校准完成前禁止中后期光学停顿重绑，避免相机重绑反复清空逐格校准。早期坏相位重定相仍最多一次，四码 30 FPS 不受影响。
- `0.8.132` 修复 0.8.131 补扫仍受偏置 ROI 和推算格框约束的问题：校准探测始终基于完整 1440 中心方形，对未校准象限启用备用二值化；真实命中按固定物理象限归属并直接替换对应格框，不再要求落入旧推算框。4/4 前同时禁止早期和中后期相机重绑，避免校准被清空。
- 网页接收端 `v88` 仅接受单码与四码 AFL2，双码帧明确拒绝。修复由两个真实格位推算出的虚拟格位被错误标记为已证实并立即冻结的问题：四格必须分别真实命中后才冻结，单格命中也可继续补齐校准；四码最多允许两帧解码在途以吸收偶发 worker 延迟，同时保持最新帧采集不排队。
