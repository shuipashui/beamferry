...
| 网页接收端 | `v92`，四码稳定格缓存、3 帧有界流水线、定向缺格补扫 |
...
发送端将连续序号轮转到四个物理格位，避免固定盲格永久吞掉同一 modulo-4 源符号通道。网页接收端使用受限并发的 Web Worker 和 ZXing WASM，只保留最新待分析帧。`v92` 保持桌面接收端验证稳定的 3 帧有界流水线和 Worker 预占，并借鉴原生端的稳定四格缓存与逐格恢复：连续 6 个不完整帧后，只对漏失最多的 2 格执行一次定向补扫；补扫结果不会把未扫描格计为漏失。Android 浏览器在硬件线程至少 6、报告内存至少 4 GB 时使用 3 Worker 和 720 图集，否则使用 2 Worker 和 680 图集。Android 原生端使用 CameraX Y 平面和 zxing-cpp，四格裁剪通过 JNI 解码后以单批次提交协议层。
