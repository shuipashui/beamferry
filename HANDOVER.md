...
- `0.8.133` 相机坏相位重绑时保留已验证的 4/4 稳定格框与校准状态，避免重绑后重新跌回 3/4。未完成校准时全局探测从每 8 帧提高到每 4 帧，普通/备用二值化交替；4/4 前关闭小框单格加强，避免与全局校准竞争 CPU。四码 30 FPS 不受影响。
- 四码 50/60 FPS 发送调度在首轮 systematic 完成后，以 1:7 比例均匀穿插第二遍 systematic 与新 repair；重播顺序使用与 K 互质的步长遍历全部源块。用于补偿 Android 四格校准耗时波动导致的首轮源块缺失和进度启动停顿。四码 30、双码和单码调度不变。
- `0.8.134` 将高速四码经真实命中校准的格框扩边从 18% 增至 35%，改善“已校准但单格长期低命中”和频繁备用二值化；只作用于 50/60 FPS。systematic 重播由 1:3 降至 1:7，减少快速校准会话中的重复开销。重定相保留校准后，统计计数器重置不再产生大于 4 QR/frame 的无效前后对比。
