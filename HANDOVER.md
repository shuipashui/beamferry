...
**0.8.89：** 去掉空扫 HAL 杀进程倒计时。对准二维码时唯一帧 1–4、空扫仍高，0.8.88 仍会弹出「正在释放相机」。自动恢复只留软解码；倒计时只留给用户点「继续接收」且相机未绑。
...
| Android APK | **0.8.89**（versionCode 103） | 无 HAL 杀进程倒计时；四码格4并行；继续热相机 |
