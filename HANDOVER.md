...
| 项目 | 当前值 |
|---|---|
| Android 源码版本 | `0.8.119-quad-phase-recovery` |
| versionCode | `132` |
...
- 实验四码 60 使用独立 AFL2 magic `0x1e/0x1f`；接收端据此启用稳定四格缓存、每 8 帧低频补扫、
  0/1/2/3/4 码帧统计和坏相位相机重绑。普通四码 30 仍用 `0x0d/0x0f`，不会触发这些实验恢复逻辑。
- 四码 60 连续三个有效统计窗口低于 `1.25 QR/frame` 时强制重绑 CameraX，最多三次，协议解块进度保留。
