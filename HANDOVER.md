...
| 网页接收端 | `v93`，四码稳定格缓存、3 帧有界流水线、全四格弱位补扫 |
...
发送端将连续序号轮转到四个物理格位，避免固定盲格永久吞掉同一 modulo-4 源符号通道。网页接收端使用受限并发的 Web Worker 和 ZXing WASM，只保留最新待分析帧。`v93` 保持桌面接收端验证稳定的 3 帧有界流水线和 Worker 预占，并借鉴原生端的稳定四格缓存与逐格恢复：补扫不再以双格任务替换正常四格任务；每 18 个不完整帧只扩大最弱 2 格的裁剪范围，但仍扫描全部四格。两个格位连续长期失锁时清除旧几何并重新校准。Android 浏览器在硬件线程至少 6、报告内存至少 4 GB 时使用 3 Worker 和 720 图集，否则使用 2 Worker 和 680 图集。Android 原生端使用 CameraX Y 平面和 zxing-cpp，四格裁剪通过 JNI 解码后以单批次提交协议层。
