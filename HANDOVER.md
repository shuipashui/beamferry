...
| 网页接收端 | `v94`，四码稳定格缓存、停滞门控重校准、Android 同口径测速 |
...
发送端将连续序号轮转到四个物理格位，避免固定盲格永久吞掉同一 modulo-4 源符号通道。网页接收端使用受限并发的 Web Worker 和 ZXing WASM，只保留最新待分析帧。`v94` 保持桌面接收端验证稳定的 3 帧有界流水线和 Worker 预占，并借鉴原生端的稳定四格缓存与逐格恢复：补扫不再以双格任务替换正常四格任务；每 18 个不完整帧只扩大最弱 2 格的裁剪范围，但仍扫描全部四格。只有两个格位长期失锁且唯一序列停滞至少 1.2 秒时才清除旧几何并重新校准。速度诊断对齐 Android 端，区分实时窗口、滚动均值、全程平均和总耗时；开始扫描会先清除旧传输状态。Android 浏览器在硬件线程至少 6、报告内存至少 4 GB 时使用 3 Worker 和 720 图集，否则使用 2 Worker 和 680 图集。Android 原生端使用 CameraX Y 平面和 zxing-cpp，四格裁剪通过 JNI 解码后以单批次提交协议层。
