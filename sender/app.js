(() => {
  ...
  const HEADER_LEN = 20;
  const RECEIVER_URL = "https://shuipashui.github.io/beamferry/";
  ...
  function nextFrameSeq() {
    ...
    if (quadRefreshesAll() && Number(fps.value) >= 50) {
      // Once the systematic pass ends, alternate source-block replays with
      // fountain repairs. Screen capture commonly misses whole display frames;
      // replaying only one source block in eight makes the last few percent
      // wait on LT peeling for much longer than the main transfer.
      if (tail % 2 === 0) {
        const replay = (Math.floor(tail / 2) * highReplayStep + Math.floor(transfer.total / 2)) % transfer.total;
        return (0x80000000 | replay) >>> 0;
      }
      return Math.floor(tail / 2);
    }
    return tail;
  }
