...
网页接收端只接受单码和四码 AFL2 码流。Android 接收端保留更广泛的协议兼容能力。
...
Android 接收端直接分析 CameraX 亮度平面，并将四码裁剪区域交给 zxing-cpp 解码。稳定生产配置仍为四码 30 FPS。50/60 FPS 全刷属于实验路径，具有独立的格位校准和光学相位恢复逻辑，不会改变原有 30 FPS 路径。
