...
当前发布基线：Android `0.8.135-quad-stall-classifier`（`versionCode 148`），网页接收端 `v93`。
...
网页接收端只处理最新可用视频帧，并限制 worker 并发，不会建立无限增长的解码队列。单码模式跟踪一个紧凑 ROI；四码模式分别校准四个物理格位，并且只冻结经过真实二维码命中的格位。`v93` 继续以稳定的桌面接收端流水线为基线：并行预热 Worker、最多保留 3 个相机帧任务、位图抓取前预占 Worker，并将完整校准的四格几何独立缓存。正常四格任务不再被双格补扫替换；每 18 个不完整帧仅扩大最弱两格的裁剪范围，同时仍扫描全部四格。两个格位长期失锁时会主动清除旧几何并重新校准。诊断新增剩余块、新序列/新解块间隔、唯一率和重校准次数。Android 设备在至少 6 个硬件线程且报告内存不少于 4 GB 时使用 3 Worker 和 720 像素图集；低配 Android 保持 2 Worker 和 680 像素图集。
