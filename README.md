...
当前发布基线：Android `0.8.135-quad-stall-classifier`（`versionCode 148`），网页接收端 `v89`。
...
网页接收端只处理最新可用视频帧，并限制 worker 并发，不会建立无限增长的解码队列。单码模式跟踪一个紧凑 ROI；四码模式分别校准四个物理格位，并且只冻结经过真实二维码命中的格位。`v89` 会并行预热解码 Worker，并允许最多 3 个不同相机帧进入有界四码流水线；位图抓取前会预占 Worker，旧设备的 Canvas 回退仍保持串行。
