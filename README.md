...
当前发布基线：Android `0.8.135-quad-stall-classifier`（`versionCode 148`），网页接收端 `v92`。
...
网页接收端只处理最新可用视频帧，并限制 worker 并发，不会建立无限增长的解码队列。单码模式跟踪一个紧凑 ROI；四码模式分别校准四个物理格位，并且只冻结经过真实二维码命中的格位。`v92` 以稳定的桌面接收端流水线为基线：并行预热 Worker、最多保留 3 个相机帧任务、位图抓取前预占 Worker，并将完整校准的四格几何独立缓存。正常帧继续扫描四格；连续不完整帧每 6 次最多插入一次双格定向补扫，且只有补扫启用备用二值化，避免拖慢热路径。Android 设备在至少 6 个硬件线程且报告内存不少于 4 GB 时使用 3 Worker 和 720 像素图集；低配 Android 保持 2 Worker 和 680 像素图集。
