...
当前发布基线：Android `0.8.135-quad-stall-classifier`（`versionCode 148`），网页接收端 `v91`。
...
网页接收端只处理最新可用视频帧，并限制 worker 并发，不会建立无限增长的解码队列。单码模式跟踪一个紧凑 ROI；四码模式分别校准四个物理格位，并且只冻结经过真实二维码命中的格位。`v91` 会并行预热解码 Worker，并允许最多 3 个不同相机帧进入有界四码流水线；位图抓取前会预占 Worker，旧设备的 Canvas 回退仍保持串行。Android 设备在至少 6 个硬件线程且报告内存不少于 4 GB 时使用 3 个解码 Worker，并恢复 720 像素四码图集以提高高密度二维码的模块采样；低配 Android 保持 2 Worker 和 680 像素图集。
