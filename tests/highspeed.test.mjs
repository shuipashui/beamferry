...
assert.ok(sender.includes('QR V') && sender.includes('栅格每模块 ') && sender.includes('显示 '), "sender diagnostics must expose QR density, integer raster scale, and displayed size");
assert.ok(sender.includes("const displayScale = Math.min(box.width / canvasW, box.height / canvasH);"), "denser QR versions must fill the same available display box instead of shrinking at integer-scale boundaries");
assert.ok(sender.includes("cssWidth: canvasW * displayScale") && sender.includes("cssHeight: canvasH * displayScale"), "sender canvas CSS size must use the full fitted display area");
