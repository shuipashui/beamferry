...
assert.ok(androidAnalyzer.includes("if (stable != null)"), "high-FPS quad sparse hits must not move an established grid");
...
assert.ok(androidAnalyzer.includes("preserveQuadCalibration") && androidMain.includes("preserveQuadCalibration = false"), "quad camera rephasing must reacquire the four-slot grid after a bad phase");
assert.ok(androidAnalyzer.includes("QUAD_CALIBRATED_TILE_PAD = 1.0f"), "high-FPS quad must avoid overlapping calibrated crops that duplicate neighboring codes");
assert.ok(androidAnalyzer.includes("QUAD_RECOVERY_INTERVAL = 6"), "experimental quad recovery scans must be rate limited");
assert.ok(androidAnalyzer.includes("val recoverNow = calibrateNow || count < 4"), "quad recovery must probe incomplete frames, not only complete misses");
assert.ok(androidAnalyzer.includes("if (!lockedQuad && previousTiles.size >= 4 && transferCount(merged) >= 3)"), "ordinary quad decoding must retain its immediate partial-result path");
for (const needle of [
...
assert.ok(indexHtml.includes("app.js?v=91"), "index.html must cache-bust app.js with the current receiver build");
...
assert.ok(serviceWorker.includes('const CACHE_NAME = "airferry-lite-v91";'), "service worker cache version was not bumped");
...
assert.ok(source.includes("const rollingRates = [0, 0, 0, 0, 0]") && source.includes("speedBps * 0.6 + sample * 0.4"), "speed diagnostics must smooth short camera jitter");
